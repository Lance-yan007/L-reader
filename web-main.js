...
class WebApp {
    ...
    async waitForAdapters() {
        ...
        let attempts = 0;
        while (attempts < 50) {
            if (window.FileSystemAdapter && window.StorageAdapter && window.ipcRenderer) {
    ...
    async showReaderView() {
        ...
        try {
            const response = await fetch('src/reader.html');
            const html = await response.text();
            const tempDiv = document.createElement('div');
            tempDiv.innerHTML = html;
            const bodyContent = tempDiv.querySelector('body') || tempDiv;
    ...
    async showVocabularyView() {
        ...
        try {
            const response = await fetch('src/vocabulary.html');
            const html = await response.text();
            const tempDiv = document.createElement('div');
            tempDiv.innerHTML = html;
            const bodyContent = tempDiv.querySelector('body') || tempDiv;
    ...
    async showAuthView() {
        ...
        try {
            const response = await fetch('src/auth.html');
            const html = await response.text();
            const tempDiv = document.createElement('div');
            tempDiv.innerHTML = html;
            const bodyContent = tempDiv.querySelector('body') || tempDiv;
    ...
    async loadMainScript() {
        return this.loadScript('src/scripts/main.js', true);
    }
